...
module.exports = (router) => {
    ...
    router.param('version_id', (req, res, next, id) => {
        ...
        if (req.box) {
            where.box_id = req.box.id
        }

        return BoxVersion.where(where)
            .fetch({
                'require': true
            })
            .then((version) => {
    ...
    router.param('provider_id', (req, res, next, id) => {
        ...
        if (req.version) {
            where.box_version_id = req.version.id;
        }

        return BoxVersionProvider.where(where)
            .fetch({
                'require': true
            })
            .then((provider) => {
    ...
    router.param('provider_name', (req, res, next, id) => {
